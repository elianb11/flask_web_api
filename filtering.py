...
class ContentBasedFiltering:
    ...
    def loadData(self, top=10000):
        ...
        if self.connection.is_connected():
            df = pd.read_sql(query, self.connection).drop_duplicates(subset=['book_id'], keep='first')
    ...
    def filter(self, top=10, discover=False):
        ...
        if self.connection.is_connected(): 
            tag = pd.read_sql(query, self.connection).pivot_table(index='book_id', columns='tag_name', values='perc').fillna(0)
